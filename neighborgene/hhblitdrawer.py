...
class HHBlitsDrawer(object):
    ...
    def saveSVG(self, filename, doc):
        ...
        f = open(self.hhblitResult.path + filename, 'w')
    ...
    def drawSVG(self):
        ...
        for i in range(1, stepNo + 1):
            ...
            label = ET.Element('text', x=str(leftMargin + int(i * step
                               * conversion) - int(len(str(i * step))
                               * fontSize * 0.3)), y=str(y - 10),
            ...
            doc.append(ruler)
            doc.append(label)
        startLabel = ET.Element('text', x=str(leftMargin - 10), y=str(y + 5),
        ...
        endLabel = ET.Element('text', x=str(leftMargin + int(length * conversion) + 5),
                              y=str(y + 5),
                              style='font-family:Sans-Serif;font-weight:bold;font-size:16px;text-anchor:center;dominant-baseline:top'
                              , fill='black')
        ...
        doc = self.secondaryFeaturesSVG(
            self.hhblitResult.features['psipred'],
            doc,
            'psipred',
            x,
            y + 15,
        ...
        doc = self.secondaryFeaturesSVG(
            self.hhblitResult.features['hhblits'],
            doc,
            '',
            x,
            y + 30,
    ...
    def singleAlignDraw(self, hit):
        ...
        while cursor < length:
            (doc, queryStart) = self.sequenceDraw(
                queryStart,
                hit.querySequence,
                hit.targetSequence,
                columnWidth,
            ...
            (doc, targetStart) = self.sequenceDraw(
                targetStart,
                hit.targetSequence,
                hit.querySequence,
                columnWidth,
            ...
            if len(hit.targetDSSPSecondary) > 0:
                doc = self.secondaryDraw(
                    hit.targetDSSPSecondary,
                    columnWidth,
                    cursor,
                    length,
                    fontSize,
    ...
    def sequenceDraw(
        ...
        '''
        Drawing single line of sequence in alignments
        '''
        ...
        width = fontSize * 0.7
        height = fontSize * 1.1
        realWidth = len(text.replace('-', '')) - 1
        for i in range(len(text)):
            aa = text[i]
            caa = compareText[i]
            color = self.aminoToColor(aa)
            compcolor = self.aminoToColor(caa)
            xPos = str(int(leftMargin + fontSize * i
                       * conversionFactor))
        ...
        endnumbers = ET.Element('text', x=str(leftMargin + (len(text)
                                + 3) * fontSize * conversionFactor),
                                y=str(y), fill='black',
                                style='font-family:Courier;font-size:'
                                + str(fontSize)
                                + 'px;text-anchor:end;dominant-baseline:middle'
    ...
    def aminoToColor(self, aa):
        '''
        assign color based on the amino acids characters
        '''
        # Hydrophobic
