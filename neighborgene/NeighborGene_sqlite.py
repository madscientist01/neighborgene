...
def esl_sfetch(multifasta, path, extractlist, extractedFile):
    '''
    Extract multifasta using easel-sftech utility in hmmer with fasta headers
    '''
    ...
    def run(self):
        ...
        if os.path.exists(db):
            ...
            for hit in hmmSearch.hits:
                ...
                c.execute('SELECT gff.organism, gff.start, gff.end, gff.direction, gff.protein FROM gff WHERE protein=?',t)
                for row in c.fetchall():
                    ...
                    #
                    query = "SELECT gff.organism, gff.start, gff.end, gff.direction, gff.protein, accession.accession FROM gff,accession WHERE gff.start BETWEEN ? AND ? AND gff.organism=? AND gff.protein=accession.protein" 
